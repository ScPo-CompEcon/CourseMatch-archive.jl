...
@testset "demand function tests" begin
    @testset "unimodal preferences" begin
	    #Here I have one student, 3 available classes, one being striclty preferred to the other two and it is required o enroll in one class in total.
	    #The student has the budget such that they can afford any class under the randomly chosen prices. The test hecks if the demand functions does indeed
	    ...
	    @test solution[:total] == [1, 0, 0]
    end

    @testset "unaffordable most preferred course" begin
	    #Here I have one student, 3 available classes, and it is required to enroll in one class in total. The student as a stric ordering on the three classes
